...
/**
 * @Description: 用于主数据相关的明细数据类型定义，比如个人的地址信息。
 * 有多条，但又不是像严格的单据明细
 * 因此独立出一个类型，进行维护
 * @Author: newmannhu@qq.com
 * @Date: Created in 2018-04-15 9:22
 **/
export abstract class BylListComponentEntityDetail<T extends BylBaseModel > implements OnInit {

    public masterId:string; //主数据的id
    ...
    // @Output()
    // deleteItem: EventEmitter<T> = new EventEmitter<T>();
    ...
    delete(deleteItem: T) {
        ...
        this.businessService.delete(deleteItem)
            .subscribe(data => {
                    // this._loading = false;
    ...
    search() {
        ...
        this.findByMasterId(this.masterId).subscribe(
            data => {
                this.loading = false;
                if (data.code === BylResultBody.RESULT_CODE_SUCCESS) {
                    // 正确获取数据
    ...
    processModifyItemResult(modifyResult: T){
        // this.masterModifyDateTime = modifyResult.modifyDateTime;
        this.updateListData( modifyResult);
    ...
    abstract findByMasterId(masterId: string):Observable<BylResultBody<Array<T>>> ;
