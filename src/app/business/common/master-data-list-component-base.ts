...
/**
 * @Description: 主数据list组件的抽象类
 * @Author: newmannhu@qq.com
 * @Date: Created in 2018-04-15 9:22
 **/
export abstract class BylMasterDataListComponentBasePro<T extends BylMasterDataBaseModel>
    ...
    showUnconfirmEntity(entity: any){
        this.modalService.confirm({
            nzTitle: '确认要进行取消确认操作吗?',
            nzContent: '<b style="color: red;">取消确认之后，该记录退回到提交状态，进行修改调整，但不能在其他单据中使用。</b>',
            nzOkText: '确认',
            nzOkType: 'primary',
            nzOnOk: () => {
                this.unconfirmEntity(entity);
            },
            nzCancelText: '取消',
            nzOnCancel: () => console.log('showConfirmEntity Cancel')
        });
    ...
    unconfirmEntity(entity: any) {
        this.actionResult$ = this.businessService.unconfirm(entity);
        this.actionFollowProcess(this.actionResult$);
    }
    ...
    entityAction(action: BylTableClickAction){
        switch(action.actionName){
            ...
            case ACTION_CONFIRM:
                ...
                break;
            case ACTION_UNCONFIRM:
                this.showUnconfirmEntity(action.rowItem);
                break;
