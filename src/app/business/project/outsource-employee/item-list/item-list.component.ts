...
import {BylOutsourceEmployeeAvailablePoolsInterface} from "../../../../service/project/service/outsource-employee-related.interface";
...
@Component({
    selector: 'byl-outsource-employee-item-list',
    templateUrl: './item-list.component.html',
})
export class BylOutsourceEmployeeItemListComponent
    extends BylListComponentBasePro<BylOutsourceEmployee>{
    ...
    // private _outsourcerId: string;

    @Input()
    set outsourcerWidget(value: any){
        // this._outsourcerId = value;
            this.queryDefaultData.outsourcerWidget = value;
    }

    @Input() findAvailablePoolsService: BylOutsourceEmployeeAvailablePoolsInterface; //调用方，用户调出选择添加账户的窗口
    //
    ...
    search() {
        ...
        this.findAvailablePoolsService.findAvailableOutsourceEmployeePoolsPage(this.genQueryModel(), this.page, this._masterId).subscribe(
            data => {
                this.loading = false;
...
//#endregion
    ...
    addSelected(){
        let  result:Array<BylOutsourceEmployee>;
